...
public class MappingCache {
    ...
    private static TableMetaData initCache(CommandTable commandTable, MappingHandler mappingHandler) {
        ...
        if (aTable != null) {
            ...
            //如果有分表数据，不能使用缓存
            tableMetaData.setTableName(mappingHandler.getTableName(modelClass, null));
        }
        ...
        for (Field field : beanFields) {
            ...
            if (column == null) {
                classFields.put(field.getName(), mappingHandler.getColumnName(modelClass, field.getName()));
            } else {
        ...
        if (StringUtils.isBlank(pkField)) {
            pkField = mappingHandler.getPkFieldName(modelClass);
        }
